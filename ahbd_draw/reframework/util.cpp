...
reframework::API::ManagedObject* util::get_main_view() {
    ...
    static auto main_view = get_main_view_method->call<reframework::API::ManagedObject*>(api->sdk()->functions->get_vm_context(), api->get_native_singleton("via.SceneManager"));
...
reframework::API::ManagedObject* util::get_primary_camera() {
    ...
    static auto camera = get_primary_camera_method->call<reframework::API::ManagedObject*>(api->sdk()->functions->get_vm_context(), main_view);
...
reframework::API::ManagedObject* util::get_current_scene() {
    ...
    static auto scene = get_current_scene_method->call<reframework::API::ManagedObject*>(api->sdk()->functions->get_vm_context(), api->get_native_singleton("via.SceneManager"));
...
glm::quat util::get_joint_rotation(reframework::API::ManagedObject* joint) {
    const auto& api = reframework::API::get();
    static auto get_rotation_method = api->tdb()->find_type("via.Joint")->find_method("get_Rotation");

    glm::quat rotation{};
    get_rotation_method->call<reframework::API::ManagedObject*>(&rotation, api->sdk()->functions->get_vm_context(), joint);

    return rotation;
}

Vector4f util::get_transform_position(reframework::API::ManagedObject* transform) {
    auto& api = reframework::API::get();
    const auto tdb = api->tdb();

    static auto transform_def = tdb->find_type("via.Transform");
    static auto get_position_method = transform_def->find_method("get_Position");

    Vector4f pos{};
    get_position_method->call<Vector4f*>(&pos, api->sdk()->functions->get_vm_context(), transform);
...
std::optional<Vector3f> util::get_camera_up() {
    ...
    static auto transform_def = tdb->find_type("via.Transform");
    static auto get_gameobject_method = transform_def->find_method("get_GameObject");
    ...
    static auto get_transform_method = gameobject_def->find_method("get_Transform");

    auto camera_gameobject = get_gameobject_method->call<reframework::API::ManagedObject*>(context, camera);

    if (camera_gameobject == nullptr) {
        return std::nullopt;
    }

    auto camera_transform = get_transform_method->call<reframework::API::ManagedObject*>(context, camera_gameobject);

    if (camera_transform == nullptr) {
        return std::nullopt;
    }
    auto camera_joints = get_joints_method->call<reframework::API::ManagedObject*>(context, camera_transform);
    ...
    auto camera_joint = get_item_method->call<reframework::API::ManagedObject*>(context, camera_joints, 0);
...
std::optional<Vector2f> util::world_to_screen(const Vector3f& world_pos) {
    ...
    static auto world_to_screen = math_t->find_method("worldPos2ScreenPos(via.vec3, via.mat4, via.mat4, via.Size)");
    ...
    world_to_screen->call(&screen_pos, context, &pos, &g_camera.view, &g_camera.proj, &g_camera.screen_size);
...
bool util::update_camera() {
    ...
    main_view->call("get_WindowSize", &g_camera.screen_size, context, main_view);
